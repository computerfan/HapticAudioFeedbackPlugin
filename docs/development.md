...
Mac system-default capture checks the current CPAL device ID every 500 ms inside the existing helper and closes the old stream when it changes. The plugin attempts a restart after 500 ms for runtime failures of default sources; it does not automatically retry Mac startup failures or permission denials. If the new route is not ready or unsupported, use the capture retry button or toggle haptics off and on. Explicit device selections do not follow the default. Switching haptics from off to on restarts capture only when it has an error, is absent, or has received no packets within the last second. A healthy stream, including silent PCM, is preserved. This applies on both platforms, through the shared settings path used by browser controls and SDK actions. This implementation has automated policy checks but still requires real Mac routing/permission validation.
