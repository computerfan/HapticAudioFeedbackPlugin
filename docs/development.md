...
CPAL 0.18.2 provides WASAPI capture on Windows and CoreAudio capture on macOS. Device choices use stable, direction-qualified IDs. The system default is resolved when capture starts. On Windows, a default-device change or capture failure automatically reopens system-default playback/input capture after 500 ms, with retries backing off to 30 seconds while unavailable. Receiving audio resets the retry delay. Explicit device selections and permission-denied errors do not trigger automatic recovery. An unavailable explicitly selected device fails without switching sources.
...
See [Plugin configuration and data folder](../README.md#plugin-configuration-and-data-folder) for Windows and macOS paths and instructions for opening them. The plugin obtains the actual directory from the SDK's [`GetPluginDataDirectory()`](https://logitech.github.io/actions-sdk-docs/csharp/plugin-features/storing-plugin-data/). It places `Open Haptic Settings.html` and the `logs` subfolder there. The macOS path was also shown in the user-provided capture logs. Use the SDK-returned directory as authoritative if a host installation uses a different location.
...
The detector records the qualifying trigger rule before changing its armed state. Sent trace frames carry `TriggerReason`: `threshold` draws a circle and `rise` draws a diamond. If both rules qualify, the level rule takes precedence; a rapid-rise marker means the rapid-rise rule was needed. Line and marker-outline colors identify the band. Marker fill and numbers 1–6 identify the sent texture using a fixed legend. Trace frames retain `SentTexture`, resolved from the actual sent event, so later profile edits do not recolor history. Unknown textures draw hollow markers; unknown trigger reasons use circles. This annotation does not change onset selection, timing, or scheduling.
...
Mac system-default capture checks the current CPAL device ID every 500 ms inside the existing helper and closes the old stream when it changes. The plugin attempts a restart after 500 ms for runtime failures of default sources; it does not automatically retry Mac startup failures or permission denials. If the new route is not ready or unsupported, use the capture retry button or toggle haptics off and on. Explicit device selections do not follow the default. Turning haptics off closes capture, releases detector buffers, and cancels pending startup/recovery. Turning it back on opens a fresh stream. Source changes and manual retries while off keep capture paused. This applies on both platforms, through the shared settings path used by browser controls and SDK actions. This implementation has automated policy checks but still requires real Mac routing/permission validation.
...
Spectral vibrato suppression uses the maximum previous magnitude within ±0–3 FFT bins before calculating positive differences. Zero preserves ordinary flux. It uses past data only and no additional buffers; bin width varies with sample rate/FFT size. Larger neighborhoods can suppress nearby genuine note changes. The experimental profile enables radius 1 on detail; legacy settings remain at 0. This is an independently implemented maximum-neighborhood variant, not a claim of reproducing the complete SuperFlux method or its published accuracy.
