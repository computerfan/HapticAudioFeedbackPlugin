...
function harness(){
 ...
 const defaults={BassDetectionMethod:'envelope',HighDetectionMethod:'envelope',BassSpectralThreshold:.12,HighSpectralThreshold:.12,Enabled:false,EnableDebugServer:false,Sensitivity:50,CaptureDeviceId:'',BassGainDb:0,HighGainDb:0,BassEnabled:true,HighEnabled:true,LowCenterHz:100,HighCenterHz:2000,LowThresholdDb:-38,HighThresholdDb:-42,OnsetMarginDb:6,RearmMarginDb:2,AttackMilliseconds:5,ReleaseMilliseconds:60,BackgroundMilliseconds:300,MinimumSpacingMilliseconds:80,MaximumEventAgeMilliseconds:50,OnsetRiseDb:3,BassTriggerMode:'both',BassFilterQ:1.2,BassAttackMilliseconds:5,BassReleaseMilliseconds:60,BassOnsetRiseWindowMilliseconds:20,BassTransientSeparationMilliseconds:80,HighTriggerMode:'both',HighFilterQ:1.6,HighAttackMilliseconds:5,HighReleaseMilliseconds:60,HighOnsetRiseWindowMilliseconds:20,HighTransientSeparationMilliseconds:80,OnsetRiseWindowMilliseconds:20,TransientSeparationMilliseconds:80,StrongBassAboveThresholdDb:12,BassWaveform:'damp_collision',StrongBassWaveform:'sharp_collision',HighWaveform:'subtle_collision',SustainEnabled:false,SustainWaveform:'subtle_collision',SustainThresholdDb:-30,SustainSlowIntervalMilliseconds:260,SustainFastIntervalMilliseconds:140};
...
(async()=>{
 ...
 h.change('BassReleaseMilliseconds',1000);assert.equal(run('settings.BackgroundMilliseconds'),1050);
 ...
 const expert=harness();await expert.run('init()');
 expert.change('BassTriggerMode','level');assert.equal(expert.el('BassOnsetRiseWindowMilliseconds').disabled,true);assert.equal(expert.el('HighOnsetRiseWindowMilliseconds').disabled,false);
 ...
 expert.change('HighDetectionMethod','spectral');assert.equal(expert.el('HighSpectralThreshold').disabled,false);assert.equal(expert.el('HighTriggerMode').disabled,true);assert.equal(expert.el('HighOnsetRiseWindowMilliseconds').disabled,true);
